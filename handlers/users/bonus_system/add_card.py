...
@dp.callback_query_handler(post_callback.filter(action='add_card'))
async def enter_card(call: CallbackQuery):
    ...
    await Bonus.EnterCardWithoutGT_check.set()


@dp.message_handler(state=Bonus.EnterCardWithoutGT_check, content_types='text')
                ...
                if new_card not in card_list:
                    ...
                    if len(card_list) != 0:
                        ...
                        try:
                            ...
                            await message.answer(f'Мої бонуси: <code>{ruan.check_balance(new_card)}</code> балів',
                                                 reply_markup=main_keyboard)
                            await state.finish()
